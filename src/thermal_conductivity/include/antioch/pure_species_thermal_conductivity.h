...
namespace Antioch{
  ...
  class PureSpeciesThermalConductivity
  {
      public:
        ...
        ANTIOCH_AUTOFUNC(StateType, mu * _thermo.cv_trans(s) * five_over_two * (one - two_over_pi * _thermo.cv_rot(s) / _thermo.cv_trans(s) * this->A(rho * Dss / mu) / this->B(s, T, rho * Dss / mu)))
